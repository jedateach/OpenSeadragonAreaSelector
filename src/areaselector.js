(function ($) {
  $.AreaSelector = function (options) {
    ...
    this.viewer.addHandler(
      "canvas-drag-end",
      $.delegate(this, this.dragEndHandler)
    );
  };
  ...
  $.extend($.AreaSelector.prototype, $.EventSource.prototype, {
    ...
    makeResizeHandles: function () {
      var handles = {},
        ...
        halfHandleSize = Math.ceil(this.handleSize / 2) + 1;
      ...
      for (var l in hlocs) {
        ...
        if (hlocs[l].t !== undefined) {
          ...
          if (hlocs[l].t === "50%") {
            el.style.marginTop = -halfHandleSize + "px";
          }
        }
        if (hlocs[l].b !== undefined) {
          el.style.bottom = hlocs[l].b;
          if (hlocs[l].b === "50%") {
            el.style.marginBottom = -halfHandleSize + "px";
          }
        }
        if (hlocs[l].l !== undefined) {
          el.style.left = hlocs[l].l;
          if (hlocs[l].l === "50%") {
            el.style.marginLeft = -halfHandleSize + "px";
          }
        }
        if (hlocs[l].r !== undefined) {
          el.style.right = hlocs[l].r;
          if (hlocs[l].r === "50%") {
            el.style.marginRight = -halfHandleSize + "px";
          }
        }
    ...
    snapToGrid: function () {
      if (this.gridWidth) {
        var gridWidth = 1 / this.gridWidth;
        this.rect.x = Math.round(this.rect.x * gridWidth) / gridWidth;
        this.rect.width = Math.round(this.rect.width * gridWidth) / gridWidth;
        //keep the size at least as big as the grid
        if (this.rect.width < this.gridWidth) {
          this.rect.width = this.gridWidth;
        }
      }
      if (this.gridHeight) {
        var gridHeight = 1 / this.gridHeight;
        this.rect.y = Math.round(this.rect.y * gridHeight) / gridHeight;
        this.rect.height =
          Math.round(this.rect.height * gridHeight) / gridHeight;
        //keep the size at least as big as the grid
        if (this.rect.height < this.gridHeight) {
          this.rect.height = this.gridHeight;
        }
      }
    },
